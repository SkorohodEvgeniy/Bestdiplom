...
(function ($) {
    if($.fn.ajaxForm == undefined) {
        $.getScript("/cabinet/js/fancybox/jquery.form.js");
    }
    var feature = {};
    feature.fileapi = $("<input name='userfile' type='file'/>").get(0).files !== undefined;
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            onSuccess: function (files, response, xhr, pd) {
				if(response=="err")alert("Ошибка загрузки, сервер сообщил о непустимом формате");
				else $("#my_files").append("<input name='my_files[]' hidden value='"+response+"'>");
			},
            ...
            var options = {
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.returnType == "json" && $.type(data) == "object" && data.hasOwnProperty(s.customErrorKeyStr)) {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span class='" + s.errorClass + "'>Ошибка: " + msg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
                ...
                error: function (xhr, status, errMsg) {
                    ...
                    if(xhr.statusText == "abort") //we aborted it
                    {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span class='" + s.errorClass + "'>Ошибка: " + errMsg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
