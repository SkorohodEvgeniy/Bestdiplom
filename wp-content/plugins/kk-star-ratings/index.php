...
if(!class_exists('BhittaniPlugin_kkStarRatings')) :
    ...
    class BhittaniPlugin_kkStarRatings extends BhittaniPlugin
    {
        ...
        */
        ...
        public function markup($id=false)
        {
            ...
            global $wp;
            $sc_title = get_the_title($id);
            $sc_url = home_url( $wp->request );
            $sc_image = "https://bestdiplom.kz/wp-content/uploads/2018/02/logotype.png";
            $sc_date = date('Y-m-d');
            $sc_price = get_field('price_sh_get', $id) ? get_field('price_sh_get', $id) : '100';

            $ratings_data = $this->extract_ratings($id);

            $sc_rate_val = $ratings_data['avg'] ? $ratings_data['avg'] : '5';
            $sc_rate_count = $ratings_data['votes'] ? $ratings_data['votes'] : '1';
            
           if(get_field('price_sh_get', $id)){
            $markup .= <<<HTML
            <script type="application/ld+json">
    {
        ...
        "worstRating":  "1",
        "bestRating": "5",
        "ratingValue": "{$sc_rate_val}",
        "ratingCount": "{$sc_rate_count}"
        },
         "offers": {
          "@type": "AggregateOffer",
          "lowprice": "{$sc_price}",
